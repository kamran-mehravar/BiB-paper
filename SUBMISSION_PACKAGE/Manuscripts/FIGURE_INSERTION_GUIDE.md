...
Use only the `_FINAL.png` files in `SUBMISSION_PACKAGE/Figures/`. The current clean and highlighted DOCX files already contain byte-identical embedded images for Figures 3-7 when compared with the `_FINAL.png` files. Manual replacement is therefore not required unless Microsoft Word displays a problem or you intentionally want to reinsert the images.
...
### Manuscript location
Section 3.2, immediately above the caption paragraph beginning "Figure 3." Insert after the paragraph ending "they do not demonstrate that palletisation and temperature act independently or exclusively on separate parts of the system."
...
### Reviewer relevance
Reviewer 2 and Reviewer 3: pressure ?P summaries, small-n exploratory ANOVA, and top/bottom position interpretation.
