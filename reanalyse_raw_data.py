...
MAIN_SENSORS = [
    # sensor, position, nominal chamber, temperature column index, pressure column index
    ("V50", "Top", "50", 2, 3),
    ("V53", "Top", "50", 4, 5),
    ("V71", "Top", "50", 6, 7),
    ("V51", "Bottom", "19", 8, 9),
...
def f_survival(f_value: float, df_num: int, df_den: int) -> float:
    """Small dependency-free F survival function used for audit reproducibility."""
...
def fit_lm(data: pd.DataFrame, response: str, terms: list[str]):
    ...
    if "interaction" in terms:
        cols.append(
            (
                (data["position"].values == "Top")
                & (data["chamber"].astype(str).values == "50")
            ).astype(float)
        )
...
def type2_anova(data: pd.DataFrame, response: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rows = []
    for factor, reduced_sse in [
        ("position", sse_no_position),
        ("chamber", sse_no_chamber),
    ]:
        ss = reduced_sse - sse
        f_value = ss / mse
        rows.append(
    ...
    rows.append(
        {
            "response": response,
            "factor": "position:chamber",
            "sum_sq": sse - sse_full,
            "df_num": 1,
            "df_den": df_full,
            "F": f_interaction,
            "p": f_survival(f_interaction, 1, df_full),
        }
    )
    ...
    contrasts = []
    for label, c in [
        ("bottom_minus_top", np.array([0.0, -1.0, 0.0])),
...
def summarize_main_pressure(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def summarize_main_temperature(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group = (
        per_sensor.groupby(["experiment", "nominal_chamber_degC", "position"])
        .agg(
            n=("sensor", "count"),
...
def load_verification_compare() -> pd.DataFrame:
    ...
    ws = wb["Compare"]
    headers = [
        ws.cell(1, col_idx).value if ws.cell(1, col_idx).value is not None else f"blank{col_idx}"
        ...
        [
            {
                "sensor": "V26",
                "position": "Top",
                "source_temperature_column": "V26_TOP_T",
                "source_pressure_column": "V26_TOP_P",
                "power_on": "2024-12-16 16:10",
                "stack": "single high-temperature three-box stack",
            },
            {
                "sensor": "V27",
                "position": "Bottom",
                "source_temperature_column": "V27_BOT_T",
                "source_pressure_column": "V27_BOT_P",
                "power_on": "2024-12-16 16:10",
                "stack": "single high-temperature three-box stack",
            },
            {
                "sensor": "V62",
                "position": "Top",
                "source_temperature_column": "V62_TOP_T",
                "source_pressure_column": "V62_TOP_P",
                "power_on": "2024-12-16 16:20",
                "stack": "single high-temperature three-box stack",
            },
            {
                "sensor": "V64",
                "position": "Bottom",
                "source_temperature_column": "V64_BOT_T",
                "source_pressure_column": "V64_BOT_P",
                "power_on": "2024-12-16 16:20",
                "stack": "single high-temperature three-box stack",
            },
        ]
            ...
            pd.DataFrame(
                {
                    ...
                    "position": position,
                    "chamber": chamber,
                    "temperature_degC": df.iloc[:, t_col],
                    "pressure_mbar": df.iloc[:, p_col],
                }
            )
        )
...
def plot_mean_sd(ax, series: pd.DataFrame, value: str, chamber: str, position: str, color: str, label: str):
    ...
    x = grouped["time_days"].to_numpy(float)
...
def regenerate_figures(main_df: pd.DataFrame, pressure_summary: pd.DataFrame, verification_df: pd.DataFrame):
    ...
    for col, chamber, title in [
        ...
        axp = axes[0, col]
        axt = axes[1, col]
        plot_mean_sd(axp, series, "pressure_mbar", chamber, "Top", amber, "Top BiB")
        plot_mean_sd(axp, series, "pressure_mbar", chamber, "Bottom", blue, "Bottom BiB")
    ...
    for col, chamber, title in [(0, "50", "(A) Nominal 50 °C chamber"), (1, "19", "(B) Nominal 19 °C chamber")]:
        for row, value, ylabel in [(0, "pressure_mbar", "Pressure (mbar)"), (1, "temperature_degC", "Temperature (°C)")]:
            ...
            for pos, color, label in [("Top", amber, "Top BiB"), ("Bottom", blue, "Bottom BiB")]:
                subset = early[(early["chamber"] == chamber) & (early["position"] == pos)]
                grouped = subset.groupby("time_h")[value].agg(["mean", "std"]).reset_index()
                x = grouped["time_h"].to_numpy(float)
                mean = grouped["mean"].to_numpy(float)
                sd = grouped["std"].fillna(0).to_numpy(float)
                ax.plot(x, mean, marker="o", color=color, linewidth=2.0, markersize=3.5, label=label)
                ax.fill_between(x, mean - sd, mean + sd, color=color, alpha=0.2)
    ...
    for ax, (response, title) in zip(axes, responses):
        for chamber in ["19", "50"]:
            for position, color in [("Top", amber), ("Bottom", blue)]:
                vals = pressure_summary[
                    (pressure_summary["chamber"] == chamber) & (pressure_summary["position"] == position)
        ...
        ax.set_xticks([0, 1])
        ax.set_xticklabels(["Nominal 19 °C", "Nominal 50 °C"])
    ...
    axes[0].legend(
        handles=[
            Line2D([0], [0], marker="o", color=amber, label="Top BiB", linestyle="", markersize=7),
