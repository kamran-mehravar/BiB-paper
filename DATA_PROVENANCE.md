...
| `go_no_go_audit.py` | Final scientific rescue audit | Derived-output script; does not modify source files | Common-mode pressure correlations, common-mode residual endpoints, leave-one-out pressure sensitivity, verification thermal response approximations | Sensor-level summaries and traces | Uses explicit sensor maps through `reanalyse_raw_data.py` | Main stack IDs unavailable; verification is one stack | Main 30 min; verification 1 min | Supports final common-mode pressure caveat, thermal interpretation and `GO_NO_GO_REVIEW.md` |
